    ...
    public void handle(TransactionContext ctx) {
        if (ctx.inbound() instanceof TransactionRequest) {
            ...
            if (msg.getType() == TransactionType.REQUEST_IAT_UPLOAD) {
                ...
                if (ds == null) {
                    critical.error("Received a deployment request for a deployment that does not exist.  Deployment ID: "
                            + ctx.deploymentId());
                    ctx.reply().send(new TransactionRequest(TransactionType.TRANSACTION_FAIL));
                } else {
                    ds.generateTest();
                ...
                if (test == null) {
                    ...
                    try {
                        ...
                        ctx.sessionState().setWantedManifestType(ManifestType.FILE_MANIFEST);
                ...
                } else if (ctx.wantedManifestType() == ManifestType.ITEM_SLIDE_MANIFEST) {
