...
public class PasswordHandler implements TransactionHandler {
    ...
    static public final Cache<String, Long> authTokenCache = Caffeine.newBuilder()
    .maximumSize(10_000)
    ...
    public void handle(TransactionContext ctx) {
        ...
        if (transaction.getType() == TransactionType.REQUEST_ENCRYPTION_KEY) {
            ...
            ctx.reply().send(key);
        } else if (transaction.getType() == TransactionType.PASSWORD_VALID) {
            Long authToken = System.currentTimeMillis();
