    ...
    <xsl:element name="Class">
      ...
      <xsl:element name="Constructor">
        ...
        <xsl:variable name="constructorBodyElems">
          ...
          <xsl:element name="Code">
            <xsl:value-of select="concat('this.leftResponseKey = (function(v){ v = String(v == null ? &quot;&quot; : v); return /^\d+$/.test(v) ? String.fromCharCode(parseInt(v,10)) : v; })(&quot;', //LeftResponseKey, '&quot;);')"/>
          </xsl:element>
          <xsl:element name="Code">
            <xsl:value-of select="concat('this.rightResponseKey = (function(v){ v = String(v == null ? &quot;&quot; : v); return /^\d+$/.test(v) ? String.fromCharCode(parseInt(v,10)) : v; })(&quot;', //RightResponseKey, '&quot;);')"/>
          </xsl:element>
    ...
    <xsl:element name="Class">
      ...
      <xsl:element name="PrototypeChain">
        <xsl:element name="Function">
          ...
          <xsl:variable name="functionBodyElems">
            <xsl:text>
                        var pressed = (event.key &amp;&amp; event.key.length === 1) ? event.key.toLowerCase() : String.fromCharCode(event.keyCode || event.which || 0).toLowerCase();
                        var left = String(Display.getLeftResponse() || "").toLowerCase();
                        var right = String(Display.getRightResponse() || "").toLowerCase();
    ...
    <xsl:element name="Function">
      ...
      <xsl:variable name="functionBodyElems">
        <xsl:for-each select="$root//DisplayItem">
          ...
          <xsl:element name="Code">
            <xsl:value-of select="concat('DI', position(), ' = new IATDI(', ID, ', img', ID, ', ', X, ', ', Y, ', ', Width, ', ', Height, ');')"/>
          </xsl:element>
          <xsl:if test="ID = //ErrorMarkID">
            <xsl:element name="Code">
              <xsl:value-of select="concat('ErrorMark = DI', $ctx/position(), ';')"/>
            </xsl:element>
            <xsl:element name="Code">
  ...
  <xsl:template name="GenerateProcessItemFunctions">
    <xsl:for-each select="//BeginIATBlock">
      ...
      <xsl:element name="ProcessItemsFunctions">
        ...
        <xsl:variable name="items" select="following-sibling::Trial[position() le xs:integer($block/NumItems)]" />
        <xsl:element name="Function">
