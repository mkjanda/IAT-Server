...
body {
          ...
          padding: 10px 15px 10px 30px;
              ...
              box-sizing: border-box;
					...
					<xsl:for-each select="SurveyItem">
						...
						<xsl:value-of select="'margin-bottom: 0px;'" />
						...
						<xsl:for-each select="child::*[contains($selectionBasedResponses, name())]">
							...
							<xsl:value-of select="concat('p.response', $itemNum, '{&#x0A;')" />
						...
						<xsl:for-each select="child::*[contains($nonSelectionBasedResponses, name())]">
							...
							<xsl:value-of select="concat('div#response', $itemNum,' input {&#x0A;')" />
							...
							<xsl:value-of select="concat('div#response', $itemNum, ' p {&#x0A;')" />
							...
							<xsl:value-of select="concat('div#response', $itemNum, ' textarea {&#x0A;')" />
				...
				<div id="container">
					...
					<div id="mainContent">
						...
						<xsl:element name="ul">
							...
							<xsl:variable name="Items">
								...
								<xsl:for-each select="//Survey/child::*[(name() eq 'SurveyItem') or (name() eq 'SurveyImage')]">
									...
									<xsl:element name="SurveyItem">
										...
										<xsl:attribute name="QuestionNum" select="position() - count($root//SurveyItem[position() lt $pos][@Type ne 'Instruction'])" />

										<xsl:attribute name="ItemNum" select="position()" />
							...
							<xsl:apply-templates select="$Items/SurveyItem">

								<xsl:with-param name="questionNum" select="position()" />
	...
	<xsl:template name="GenerateCaption">
		...
		<xsl:element name="div">
			...
			<xsl:variable name="svgHeight" select="xs:integer($caption/LineHeight) * 3 div 2" />

			<xsl:variable name="fontSize" select="xs:integer($caption/FontSize) * $svgHeight div $svgViewBoxHeight" />

			<xsl:variable name="textX" select="'50%'" />

			<xsl:variable name="textY" select="$svgHeight * 5 div 7" />

			<xsl:element name="svg">

				<xsl:attribute name="viewBox" select="concat('0 0 ', $pageWidth, ' ', $svgHeight)" />
					...
					<defs>
						...
						<mask id="captionMask">
							...
							<xsl:element name="text">
								...
								<xsl:attribute name="x" select="$textX" />
	...
	<xsl:template match="SurveyItem[@Image eq 'True']">
		<xsl:element name="li">
	...
	<xsl:template match="SurveyItem[not(@Image)]">

		<xsl:variable name="questionNum" select="@QuestionNum" />

		<xsl:variable name="itemNum" select="@ItemNum" />
	...
	<xsl:template name="writeFormatCSS">
		...
		<xsl:value-of select="concat('font-size: ', xs:integer($format/FontSize) * 4 div 7, 'px;&#x0A;')" />
