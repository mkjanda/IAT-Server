...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:variable name="openingParagraphs">
    <Paragraph>
      ...
      <xsl:text>Thank you for your purchase of </xsl:text><xsl:value-of select="$purchase" /><xsl:text>. 
      ...
      <xsl:text>. Your product key, should you need to activate the software, is </xsl:text><xsl:value-of select="$root//ProductKey" />
    </Paragraph>
   </xsl:variable>
  ...
  <xsl:template match="PurchaseConfirmation">
  	...
  	<xsl:element name="head"><xsl:value-of select="'&#x0A;'" />
  	</xsl:element>
