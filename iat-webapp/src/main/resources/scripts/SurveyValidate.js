...
(function (global) {
    ...
    function ensureRadioSentinel(n, sentinel) {
        var radios = named(n);
        if (!radios.length) {
            return;
        }
        radios[0].value = sentinel;
    ...
    function validateOwner(owner, list) {
        ...
        function fail(msg) {
            if (ForceSubmit) {
                if (type === "truefalse" || type === "multichoice" || type === "likert") {
                    ensureRadioSentinel(n, FORCE_UNANSWERED);
                } else {
        ...
        function unanswered() {
            if (type === "truefalse" || type === "multichoice" || type === "likert") {
                ensureRadioSentinel(n, sentinel);
            } else {
                setFieldValue(n, sentinel);
            }
            return optional && !ForceSubmit ? 0 : (ForceSubmit ? 1 : fail("Please select a response to the question below."));
        }
        ...
        if (type === "truefalse" || type === "multichoice" || type === "likert") {
            ...
            if (!picked) {
                if (optional && !ForceSubmit) {
                    ensureRadioSentinel(n, UNANSWERED);
                    return 0;
                }
                return unanswered();
            }
